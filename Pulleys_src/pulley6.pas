{****************************************************}
{* "Horizontal pulley" unit for Pulleys application *}
{****************************************************}
...
type
  ...
  { TfPulleys6 }
  {************}
  TfPulleys6 = class(TForm)
    StaticText1: TStaticText;
    laQuestion: TLabel;
    Label8, Label9, Label10, Label11: TLabel;
    Image1: TImage;
    laM1, laM2, laMu, laMp, laA1, laA2, laT1, laT2: TLabel;
    laUA1, laUA2, laUMp: TLabel;
    edM1, edM2, edMu, edMp, edA1, edA2, edT1, edT2: TEdit;
  ...
  private
    rM1, rM2, rMu, rMp, rT1, rT2, rA1, rA2: Real;
  public
...
{ Application satrt: Apply sub- and superscripts }

procedure TfPulleys6.FormCreate(Sender: TObject);
...
procedure TfPulleys6.FormActivate(Sender: TObject);

var
  S: string;

begin
  laQuestion.Caption := 'Question ' + IntToStr(iQuestion) + ' / ' + IntToStr(iQuestions) + ': Horizontal pulley.';
  // Pulley system description, depending on kind of exercise
  ...
  if iCase = 0 then begin
    ...
    laMp.Visible := False; edMp.Visible := False; laUMp.Visible := False;
  end
  else if iCase = 1 then begin
    S := 'Mass- and frictionless pulley. ';
    ...
    S := 'Frictionless pulley, but considering its mass. ';
    S += 'The kinetic friction, acting on block 1, is neglected.';
    laMu.Visible := False; edMu.Visible := False;
  ...
  // 3 kinds of exercises corr. to selections in the main window's "Options" menu
  if iCase = 0 then begin
    // Simple case
    rA2 := rM2 * rG / (rM1 + rM2); rA1 := rA2;
    rT1 := rM1 * rA1; rT2 := rT1;
  end
  else if iCase = 1 then begin
    // Case with kinetic friction
    rMu := (Random(48) + 3) / 100; edMu.Text := FloatToStr(rMu);
    rA2 := (rM2 * rG - rMu * rM1 * rG) / (rM1 + rM2); rA1 := rA2;
    rT1 := rM1 * rA1 + rMu * rM1 * rG; rT2 := rT1;
  end
  else begin
    // Case with pulley mass considered
    rMp := (Random(17) / 2 + 2) / 20; edMp.Text := FloatToStr(rMp);
    rA2 := (rM2 * rG) / (rM1 + rM2 + rMp / 2); rA1 := rA2;
    rT1 := rM1 * rA1; rT2 := rM2 * rG - rM2 * rA2;
  end;
  edA1.SetFocus;
  bCorrect := False;
...
procedure TfPulleys6.btAnswerClick(Sender: TObject);
...
procedure TfPulleys6.btShowClick(Sender: TObject);
...
procedure TfPulleys6.btCloseClick(Sender: TObject);
