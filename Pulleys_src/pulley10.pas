{******************************************************************}
{* "Two horizontal pulleys (case 1)" unit for Pulleys application *}
{******************************************************************}
...
type
  ...
  { TfPulleys10 }
  {*************}
  TfPulleys10 = class(TForm)
    StaticText1, StaticText2: TStaticText;
    laQuestion: TLabel;
    Label9, Label10, Label11, Label12: TLabel;
    Image1: TImage;
    laM1, laM2, laA1, laA2, laT1, laT2: TLabel;
    laUA1, laUA2: TLabel;
    edM1, edM2, edA1, edA2, edT1, edT2: TEdit;
    imEval: TImage;
    btAnswer: TButton;
    btShow: TButton;
    btClose: TButton;
...
procedure TfPulleys10.FormCreate(Sender: TObject);
...
procedure TfPulleys10.FormActivate(Sender: TObject);

begin
  laQuestion.Caption := 'Question ' + IntToStr(iQuestion) + ' / ' + IntToStr(iQuestions) + ': Two horizontal pulleys (case 1).';
  edT1.Text := ''; edT2.Text := '';
  edA1.Text := ''; edA2.Text := '';
  imEval.Visible := False;
  repeat
    rM1 := Random(13) / 2 + 4; rM2 := Random(29) / 2 + 2;
    rA1 := rM2 * rG / (rM2 / 2 + 2 * rM1); rA2 := 2 * rA1;
  until rA1 <= 5;
  edM1.Text := FloatToStr(rM1); edM2.Text := FloatToStr(rM2);
  rT1 := rM1 * rA1; rT2 := rT1 / 2;
  edA1.SetFocus;
  bCorrect := False;
...
procedure TfPulleys10.btAnswerClick(Sender: TObject);
...
procedure TfPulleys10.btShowClick(Sender: TObject);
...
procedure TfPulleys10.btCloseClick(Sender: TObject);
