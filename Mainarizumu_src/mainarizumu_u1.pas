{*****************************************}
{* Main unit for Mainarizumu application *}
{*****************************************}
...
type
  ...
  TOperator = record
    Oprator: string;
    FirstR, FirstC, SecondR, SecondC: Integer;
  end;
  TOperators = array of TOperator;
  {***************}
  { TfMainarizumu }
  {***************}
  TfMainarizumu = class(TForm)
    mMenu: TMainMenu;
  ...
  private
    ...
    stOperators: array[0..15] of TStaticText;
  end;

var
  fMainarizumu: TfMainarizumu;
...
begin
  if fMainarizumu.btDone.Enabled then begin
...
procedure TfMainarizumu.FormCreate(Sender: TObject);
...
begin
  ...
  // Create comparison/arithmetic operators static texts
  for I := 0 to 15 do begin
    stOperators[I] := TStaticText.Create(stOperators[I]);
    stOperators[I].Parent := Self;
    stOperators[I].Height := 36;
    stOperators[I].Width := 20;
    stOperators[I].Alignment := taCenter;
    stOperators[I].Font.Style := [fsBold];
    stOperators[I].Font.Color := clBlue;
    stOperators[I].Visible := False;
  end;
...
procedure TfMainarizumu.mPuzzleNewClick(Sender: TObject);
...
var
  Col, N1, N2, R, R1, C1, R2, C2, I, J, K: Integer;
  OK: Boolean;
...
begin
  ...
  // Hide all operator static texts
  for I := 0 to 15 do begin
    stOperators[I].Visible := False;
    stOperators[I].Font.Size := stOperator.Font.Size;                          // default size (will be changed for fields difference)
  end;
  ...
  // Randomly generate comparison and arithmetic (difference) oprators
  repeat
    N1 := Random(iSize) + iSize - 2;                                           // number of comparison operators used in this puzzle
    N2 := Random(iSize) + iSize - 2;                                           // number of arithmetic operators used in this puzzle
  until (N1 + N2 >= 1.5 * iSize) and (N1 + N2 <= 2 * iSize);                   // limit them to some "reasonable" number
  SetLength(aOperators, N1 + N2);
  for I := 0 to N1 + N2 - 1 do begin
    // Generate the operators
    repeat
      OK := True;
      R1 := Random(iSize); C1 := Random(iSize);                                // chose random first field
      R := Random(2);
      case R of
        0: begin R2 := R1; C2 := C1 + 1; end;                                  // second field = next field in same row
        1: begin R2 := R1 + 1; C2 := C1; end;                                  // second field = next field in same column
      end;
      if (R2 > iSize - 1) or (C2 > iSize - 1) then                             // avoid fields falling outside the grid
        OK := False
      else if (I > N1 - 1) and (aGridNumbers[R1, C1] <= aGridNumbers[R2, C2]) then  // differences should always give a positive result
        OK := False
      else begin
        // Eliminate doubles
        for J := 0 to I - 1 do begin
          if (R1 = aOperators[J].FirstR) and (C1 = aOperators[J].FirstC) and (R2 = aOperators[J].SecondR) and (C2 = aOperators[J].SecondC) then
            OK := False;
        end;
      end;
    until OK;
    // Set values for actual record of TOperators array
    aOperators[I].FirstR := R1; aOperators[I].FirstC := C1;
    aOperators[I].SecondR := R2; aOperators[I].SecondC := C2;
    if I <= N1 - 1 then begin
      // Comparison operators
      if aGridNumbers[R1, C1] > aGridNumbers[R2, C2] then
        aOperators[I].Oprator := '>'
      else
        aOperators[I].Oprator := '<';
    end
    else begin
      // Arithmeetic (difference) operators
      aOperators[I].Oprator := IntToStr(aGridNumbers[R1, C1] - aGridNumbers[R2, C2]);
    end;
  end;
  ...
  for I := 0 to Length(aOperators) - 1 do begin
    if aOperators[I].FirstC <> aOperators[I].SecondC then begin
      // Operator between 2 fields in same row
      stOperators[I].Top := shGridShapes[aOperators[I].FirstR, aOperators[I].FirstC].Top + 2;
      stOperators[I].Left := shGridShapes[aOperators[I].SecondR, aOperators[I].SecondC].Left - 10;
      if (aOperators[I].Oprator = '<') or (aOperators[I].Oprator = '>') then begin
        if aGridNumbers[aOperators[I].FirstR, aOperators[I].FirstC] > aGridNumbers[aOperators[I].SecondR, aOperators[I].SecondC] then
          stOperators[I].Caption := '>'
        else
          stOperators[I].Caption := '<';
      end
      else begin
        stOperators[I].Font.Size := stOperator.Font.Size - 8;
        stOperators[I].Top := stOperators[I].Top + 10;
        stOperators[I].Caption := aOperators[I].Oprator;
      end;
    end
    else begin
      // Operator between 2 fields in same column
      stOperators[I].Left := shGridShapes[aOperators[I].FirstR, aOperators[I].FirstC].Left + 21;
      stOperators[I].Top := shGridShapes[aOperators[I].SecondR, aOperators[I].SecondC].Top - 23;
      if (aOperators[I].Oprator = '<') or (aOperators[I].Oprator = '>') then begin
        if aGridNumbers[aOperators[I].FirstR, aOperators[I].FirstC] > aGridNumbers[aOperators[I].SecondR, aOperators[I].SecondC] then
          stOperators[I].Caption := '˅'
        else
        ...
        stOperators[I].Top := stOperators[I].Top + 6;
        stOperators[I].Font.Size := stOperator.Font.Size - 8;
        stOperators[I].Caption := aOperators[I].Oprator;
      end;
    end;
    stOperators[I].Visible := True;
  end;
...
procedure TfMainarizumu.mPuzzleExitClick(Sender: TObject);
...
procedure TfMainarizumu.mOptionsSize4Click(Sender: TObject);
...
procedure TfMainarizumu.mOptionsSize5Click(Sender: TObject);
...
procedure TfMainarizumu.mOptionsSize6Click(Sender: TObject);
...
{ Menu item "Help > Source and copyright": Display source and copyright of Mainarizumu puzzle }

procedure TfMainarizumu.mHelpCopyrightClick(Sender: TObject);

var
  S: string;

begin
  S := 'I found Mainarizumu at https://www.janko.at/. This logic puzzle (also called Minarism) is a variant of Futoshiki. It may have ';
  S += 'been first published in the Japanese magazine "Puzzle Communication Nikoli" (PCN #95, Juni 2001) and is probably public domain. ';
  S += 'Concerning my "Mainarizumu" PC application itself, it is freeware/open source, as all puzzle and other applications at www.streetinfo.lu.';
  MessageDlg('Mainarizumu', S, mtInformation, [mbOK], 0);
end;

{ Menu item "Help > 'Rätsel und Puzzles' website": Point web browser to janko.at }

procedure TfMainarizumu.mHelpWebsiteClick(Sender: TObject);
...
procedure TfMainarizumu.mHelpAboutClick(Sender: TObject);

var
  S: string;

begin
  S := 'Logic game: Mainarizumu.' + LineEnding;
  S += 'Fill the grid fields with numbers from 1 to N, in a way, that all comparison relationships ';
  S += 'and difference operations are fulfilled.' + LineEnding + LineEnding;
  S += 'Version 1.0, © allu, November 2021.' + LineEnding;
  S += 'See also: Source and copyright.';
  MessageDlg('About "Mainarizumu"', S, mtInformation, [mbOK], 0);
end;

{ Button "Done": Read user solution from form grid and check if this is a correct solution }

procedure TfMainarizumu.btDoneClick(Sender: TObject);

// Solution validity check is done by checking the user values and their uniqueness per row and column, and by checking if all comparison
// relationships and field differences are fulfilled, and not by comparison of the user and generated grid numbers; in fact, there may be
// several correct solutions...

var
  Number1, Number2, I, J, I1, J1: Integer;
  Solved: Boolean;
...
begin
  ...
  // Check comparison relationships and field differences
  if Solved then begin
    for I := 0 to Length(aOperators) - 1 do begin
      Number1 := UserNumbers[aOperators[I].FirstR, aOperators[I].FirstC];
      Number2 := UserNumbers[aOperators[I].SecondR, aOperators[I].SecondC];
      if ((aOperators[I].Oprator = '>') and (Number1 < Number2))
        or ((aOperators[I].Oprator = '<') and (Number1 > Number2))
        or ((aOperators[I].Oprator[1] in ['1'..'5']) and (Number1 - Number2 <> StrToInt(aOperators[I].Oprator))) then
        Solved := False;
    end;
  end;
  // Display "success or not" message
  if Solved then
    MessageDlg('Mainarizumu', 'You have successfully solved this Mainarizumu puzzle!', mtInformation, [mbOK], 0)
  else begin
    MessageDlg('Mainarizumu', 'Sorry. Your numbers are no valid solution of this Mainarizumu puzzle !', mtInformation, [mbOK], 0);
...
procedure TfMainarizumu.btShowClick(Sender: TObject);
...
procedure TfMainarizumu.btClearAllClick(Sender: TObject);
...
procedure TfMainarizumu.bt1Click(Sender: TObject);

begin
  WriteNumber(stGridNumbers, '1', iRow, iCol);
end;

procedure TfMainarizumu.bt2Click(Sender: TObject);

begin
  WriteNumber(stGridNumbers, '2', iRow, iCol);
end;

procedure TfMainarizumu.bt3Click(Sender: TObject);

begin
  WriteNumber(stGridNumbers, '3', iRow, iCol);
end;

procedure TfMainarizumu.bt4Click(Sender: TObject);

begin
  WriteNumber(stGridNumbers, '4', iRow, iCol);
end;

procedure TfMainarizumu.bt5Click(Sender: TObject);

begin
  WriteNumber(stGridNumbers, '5', iRow, iCol);
end;

procedure TfMainarizumu.bt6Click(Sender: TObject);
...
procedure TfMainarizumu.btClearClick(Sender: TObject);
...
procedure TfMainarizumu.laGrid00Click(Sender: TObject);

begin
  SelectField(shGridShapes, 0, 0, iRow, iCol);
end;

procedure TfMainarizumu.laGrid01Click(Sender: TObject);

begin
  SelectField(shGridShapes, 0, 1, iRow, iCol);
end;

procedure TfMainarizumu.laGrid02Click(Sender: TObject);

begin
  SelectField(shGridShapes, 0, 2, iRow, iCol);
end;

procedure TfMainarizumu.laGrid03Click(Sender: TObject);

begin
  SelectField(shGridShapes, 0, 3, iRow, iCol);
end;

procedure TfMainarizumu.laGrid04Click(Sender: TObject);

begin
  SelectField(shGridShapes, 0, 4, iRow, iCol);
end;

procedure TfMainarizumu.laGrid05Click(Sender: TObject);

begin
  SelectField(shGridShapes, 0, 5, iRow, iCol);
end;

procedure TfMainarizumu.laGrid10Click(Sender: TObject);

begin
  SelectField(shGridShapes, 1, 0, iRow, iCol);
end;

procedure TfMainarizumu.laGrid11Click(Sender: TObject);

begin
  SelectField(shGridShapes, 1, 1, iRow, iCol);
end;

procedure TfMainarizumu.laGrid12Click(Sender: TObject);

begin
  SelectField(shGridShapes, 1, 2, iRow, iCol);
end;

procedure TfMainarizumu.laGrid13Click(Sender: TObject);

begin
  SelectField(shGridShapes, 1, 3, iRow, iCol);
end;

procedure TfMainarizumu.laGrid14Click(Sender: TObject);

begin
  SelectField(shGridShapes, 1, 4, iRow, iCol);
end;

procedure TfMainarizumu.laGrid15Click(Sender: TObject);

begin
  SelectField(shGridShapes, 1, 5, iRow, iCol);
end;

procedure TfMainarizumu.laGrid20Click(Sender: TObject);

begin
  SelectField(shGridShapes, 2, 0, iRow, iCol);
end;

procedure TfMainarizumu.laGrid21Click(Sender: TObject);

begin
  SelectField(shGridShapes, 2, 1, iRow, iCol);
end;

procedure TfMainarizumu.laGrid22Click(Sender: TObject);

begin
  SelectField(shGridShapes, 2, 2, iRow, iCol);
end;

procedure TfMainarizumu.laGrid23Click(Sender: TObject);

begin
  SelectField(shGridShapes, 2, 3, iRow, iCol);
end;

procedure TfMainarizumu.laGrid24Click(Sender: TObject);

begin
  SelectField(shGridShapes, 2, 4, iRow, iCol);
end;

procedure TfMainarizumu.laGrid25Click(Sender: TObject);

begin
  SelectField(shGridShapes, 2, 5, iRow, iCol);
end;

procedure TfMainarizumu.laGrid30Click(Sender: TObject);

begin
  SelectField(shGridShapes, 3, 0, iRow, iCol);
end;

procedure TfMainarizumu.laGrid31Click(Sender: TObject);

begin
  SelectField(shGridShapes, 3, 1, iRow, iCol);
end;

procedure TfMainarizumu.laGrid32Click(Sender: TObject);

begin
  SelectField(shGridShapes, 3, 2, iRow, iCol);
end;

procedure TfMainarizumu.laGrid33Click(Sender: TObject);

begin
  SelectField(shGridShapes, 3, 3, iRow, iCol);
end;

procedure TfMainarizumu.laGrid34Click(Sender: TObject);

begin
  SelectField(shGridShapes, 3, 4, iRow, iCol);
end;

procedure TfMainarizumu.laGrid35Click(Sender: TObject);

begin
  SelectField(shGridShapes, 3, 5, iRow, iCol);
end;

procedure TfMainarizumu.laGrid40Click(Sender: TObject);

begin
  SelectField(shGridShapes, 4, 0, iRow, iCol);
end;

procedure TfMainarizumu.laGrid41Click(Sender: TObject);

begin
  SelectField(shGridShapes, 4, 1, iRow, iCol);
end;

procedure TfMainarizumu.laGrid42Click(Sender: TObject);

begin
  SelectField(shGridShapes, 4, 2, iRow, iCol);
end;

procedure TfMainarizumu.laGrid43Click(Sender: TObject);

begin
  SelectField(shGridShapes, 4, 3, iRow, iCol);
end;

procedure TfMainarizumu.laGrid44Click(Sender: TObject);

begin
  SelectField(shGridShapes, 4, 4, iRow, iCol);
end;

procedure TfMainarizumu.laGrid45Click(Sender: TObject);

begin
  SelectField(shGridShapes, 4, 5, iRow, iCol);
end;

procedure TfMainarizumu.laGrid50Click(Sender: TObject);

begin
  SelectField(shGridShapes, 5, 0, iRow, iCol);
end;

procedure TfMainarizumu.laGrid51Click(Sender: TObject);

begin
  SelectField(shGridShapes, 5, 1, iRow, iCol);
end;

procedure TfMainarizumu.laGrid52Click(Sender: TObject);

begin
  SelectField(shGridShapes, 5, 2, iRow, iCol);
end;

procedure TfMainarizumu.laGrid53Click(Sender: TObject);

begin
  SelectField(shGridShapes, 5, 3, iRow, iCol);
end;

procedure TfMainarizumu.laGrid54Click(Sender: TObject);

begin
  SelectField(shGridShapes, 5, 4, iRow, iCol);
end;

procedure TfMainarizumu.laGrid55Click(Sender: TObject);
