{*****************************************}
{* Main unit for HuftierQuiz application *}
{*****************************************}
...
type
  ...
  TfHtQuiz = class(TForm)
    ...
    procedure tiHtQuizTimer(Sender: TObject);
  private
...
{ Read animals records form huftiere.dat file }
...
begin
  Assign(AnimalFile, 'huftiere.dat'); Reset(AnimalFile);
  IX := 0;
  while not EoF(AnimalFile) do begin
    Inc(IX);
    SetLength(Animals, IX);
    Read(AnimalFile, Animals[IX - 1]);                                         // fill the data of all (90) animals in an array of records
    if LeftStr(Animals[IX - 1].ShortName, 1) = '-' then
      Animals[IX - 1].ShortName := Animals[IX - 1].Name;
  end;
  Close(AnimalFile);
end;

{ Get an animal's (long or short) name }
...
begin
  if fHtQuiz.mSettingsShortNames.Checked and not fHtQuiz.mSettingsLevel4.Checked then begin
    // Short names at levels 1 - 3 (if so selected)
...
begin
  ...
  if fHtQuiz.mSettingsShortNames.Checked then begin
    // If short names are used, resort animals for combobox
  ...
  fHtQuiz.cobAnimals.Clear;
  fHtQuiz.cobAnimals.Items.AddText('-Keine Antwort-');                         // add a "no answer" item
  fHtQuiz.cobAnimals.ItemIndex := 0;
  for I := 0 to Length(AnimalList) - 1 do begin
    if (I = 0) or (AnimalList[I] <> AnimalList[I - 1]) then
      fHtQuiz.cobAnimals.Items.AddText(AnimalList[I]);
  end;
end;
...
begin
  ...
  for I := 0 to Length(AllAnimals) - 1 do begin
    ...
    P := UTF8Pos(Level, AllAnimals[I].QuizLevels);
    if P > 0 then begin
      Inc(J);
      SetLength(QuizAnimals, J); SetLength(BQuizAnimals, J);
      QuizAnimals[J - 1] := AllAnimals[I];                                     // fill in the animal record
      BQuizAnimals[J - 1] := False;                                            // array of Booleans, used to mark animals that have already been done
    end;
  end;
...
procedure TfHtQuiz.FormCreate(Sender: TObject);
...
begin
  ...
  mFileNew.Click;                                                              // reset all for new quiz
  Randomize;
end;

{ Menu item "Datei > Neuer Quiz": Reset all for new quiz }

procedure TfHtQuiz.mFileNewClick(Sender: TObject);
...
procedure TfHtQuiz.mFileSlideshowClick(Sender: TObject);
...
begin
  ...
  tiHtQuiz.Enabled := True;                                                    // enable the slideshow timer
end;

{ Menu item "Datei > Verlassen": Exit application }

procedure TfHtQuiz.mFileExitClick(Sender: TObject);
...
procedure TfHtQuiz.mSettingsPlayers1Click(Sender: TObject);
...
procedure TfHtQuiz.mSettingsPlayers2Click(Sender: TObject);
...
procedure TfHtQuiz.mSettingsLevel1Click(Sender: TObject);
...
procedure TfHtQuiz.mSettingsLevel2Click(Sender: TObject);
...
procedure TfHtQuiz.mSettingsLevel3Click(Sender: TObject);
...
procedure TfHtQuiz.mSettingsLevel4Click(Sender: TObject);

begin
  if not mSettingsLevel4.Checked then begin
    mSettingsLevel1.Checked := False; mSettingsLevel2.Checked := False;
    mSettingsLevel3.Checked := False; mSettingsLevel4.Checked := True;
    mSettingsShortNames.Checked := False; mSettingsShortNames.Enabled := False;  // no short names with level 4
    cLevel := '4';
    CreateQuiz(aAllAnimals, cLevel, aQuizAnimals, aAnimalsDone);
  end;
end;

{ Menu item "Einstellungen > Huftier-Kurznamen": Select short or long animal names }

procedure TfHtQuiz.mSettingsShortNamesClick(Sender: TObject);
...
procedure TfHtQuiz.mSettingsSlideshow1Click(Sender: TObject);
...
begin
  ...
  tiHtQuiz.Interval := 1000;
end;

procedure TfHtQuiz.mSettingsSlideshow2Click(Sender: TObject);

begin
  mSettingsSlideshow1.Checked := False; mSettingsSlideshow2.Checked := True;
  mSettingsSlideshow3.Checked := False; mSettingsSlideshow4.Checked := False;
  tiHtQuiz.Interval := 1500;
end;

procedure TfHtQuiz.mSettingsSlideshow3Click(Sender: TObject);
...
begin
  ...
  tiHtQuiz.Interval := 2000;
end;

procedure TfHtQuiz.mSettingsSlideshow4Click(Sender: TObject);

begin
  mSettingsSlideshow1.Checked := False; mSettingsSlideshow2.Checked := False;
  mSettingsSlideshow3.Checked := False; mSettingsSlideshow4.Checked := True;
  tiHtQuiz.Interval := 3000;
end;

{ Menu item "Hilfe > Hilfe": Display (very brief) help text }

procedure TfHtQuiz.mHelpHelpClick(Sender: TObject);

var
  S: string;

begin
  S := 'Anzahl der Spieler und Level wählen. Mit "Start" resp. "Frage" wird das Huftier angezeigt. Seinen ';
  S += 'Namen auswählen und "Antwort" drücken. Huftier-Kurznamen gelten nicht für Level 4. "Neuer Quiz" ';
  S += 'macht die Einstellungen wieder verfügbar.';
  MessageDlg('"Huftierquiz" Hilfe', S, mtInformation, [mbOK], 0);
end;

{ Menu item "Hilfe > Über": Display about text }

procedure TfHtQuiz.mHelpAboutClick(Sender: TObject);

var
  S: string;

begin
  S := 'Huftierquiz für 1 oder 2 Spieler.' + LineEnding + LineEnding;
  S += 'Version 1.0, © allu, Juni 2020.';
  MessageDlg('Über "Huftierquiz"', S, mtInformation, [mbOK], 0);
end;

{ Button "Start/Frage/Antwort/Stop": Quiz question/answer resp. stop of slideshow }

procedure TfHtQuiz.btStartClick(Sender: TObject);
...
begin
  ...
  if (btStart.Caption = 'Start') or (btStart.Caption = 'Frage') then begin
    ...
    stQuiz.Caption := 'QUIZ: Huftier ' + IntToStr(iQuestion1 + iQuestion2) + ' / ' + IntToStr(iQuestions);
    ...
    Filename := UTF8Trim(aQuizAnimals[iQuizQuestion].Name);
  ...
  // // Button "Antwort": Check the animal name entered by the player; update the score
    ...
    if iPlayers = 2 then begin
      if iQuestion2 <> 0 then begin                                            // avoid division by 0 with the very first question
    ...
    if iQuestion1 + iQuestion2 < iQuestions then begin
      ...
      // All animals done: Message, quiz button disable and settings re-enable
  ...
  // Button "Stop": During slideshow, use "Stop" button to terminate it
    ...
    tiHtQuiz.Enabled := False;                                                 // stop the timer
    MessageDlg('Ende der Diashow', 'Die Diashow wurde vom Benutzer abgebrochen', mtInformation, [mbOK], 0);
    mSettingsPlayers.Enabled := True; mSettingsLevel.Enabled := True;          // re-enable access to all "Einstellungen" menu items
...
procedure TfHtQuiz.btPauseClick(Sender: TObject);

begin
  // Button "Pause": Pause the slideshow
  if btPause.Caption = 'Pause' then begin
    tiHtQuiz.Enabled := False;                                                 // just disable the timer
    btPause.Caption := 'Weiter';
  end
  // Button "Weiter"; Resume the slideshow
  else begin
    btPause.Caption := 'Pause';
    tiHtQuiz.Enabled := True;                                                  // re-enable the timer
  end;
end;

{ Slideshow timer routine }

procedure TfHtQuiz.tiHtQuizTimer(Sender: TObject);
...
begin
  ...
  if iQuestion1 <= iQuestions then begin
    ...
    stSlide.Caption := 'DIASHOW: Huftier ' + IntToStr(iQuestion1) + ' / ' + IntToStr(iQuestions);
    edSlide.Text := AnimalName(aQuizAnimals, iQuestion1 - 1);
    // Display animal's image
    Filename := UTF8Trim(aQuizAnimals[iQuestion1 - 1].Name);
    Filename := './pics/' + Filename + '.jpg'; DoDirSeparators(Filename);
    imAnimal.Picture.LoadFromFile(Filename);
  end
  // Stop slideshow if all animals (for level selected) have been displayed
    ...
    tiHtQuiz.Enabled := False;                                                 // stop the timer
    if cLevel = '4' then
      MessageDlg('Ende der Diashow', 'Alle Huftierbilder wurden angezeigt', mtInformation, [mbOK], 0)
    else
      MessageDlg('Ende der Diashow', 'Alle Huftierbilder (für diesen Level) wurden angezeigt', mtInformation, [mbOK], 0);
    mSettingsPlayers.Enabled := True; mSettingsLevel.Enabled := True;          // all settings enabled again
    mSettingsPlayers.Enabled := True; mSettingsLevel.Enabled := True;          // re-enable access to all "Einstellungen" menu items
    stSlide.Visible := False; edSlide.Visible := False;                        // hide all slideshow related controls
    btStart.Enabled := False; btPause.Visible := False;
  end;
