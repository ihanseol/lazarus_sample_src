{***************************************}
{* Main unit for Futoshiki application *}
{***************************************}
...
type
  ...
  TRelationships = array of TRelationship;
  {*************}
  { TfFutoshiki }
  {*************}
  TfFutoshiki = class(TForm)
    mMenu: TMainMenu;
    ...
    StaticText1, stComparison: TStaticText;
  ...
  private
    ...
    aRelationships: TRelationships;
    shGridShapes: TNumberShapes;
    stGridNumbers: TNumberLabels;
    stComparisons: array[0..9] of TStaticText;
  end;

var
  fFutoshiki: TfFutoshiki;
...
begin
  if fFutoshiki.btDone.Enabled then begin
    // Only during game
    if GridShapes[Row, Col].Brush.Color <> clYellow then begin
      // Only for fields filled-in by user
...
procedure TfFutoshiki.FormCreate(Sender: TObject);
...
begin
  ...
  // Create relationship static texts
  for I := 0 to 9 do begin
    stComparisons[I] := TStaticText.Create(stComparisons[I]);
    stComparisons[I].Parent := Self;
    stComparisons[I].Height := 36;
    stComparisons[I].Width := 20;
    stComparisons[I].Alignment := taCenter;
    stComparisons[I].Font.Size := stComparison.Font.Size;
    stComparisons[I].Font.Style := [fsBold];
    stComparisons[I].Font.Color := clBlue;
    stComparisons[I].Visible := False;
  end;
...
procedure TfFutoshiki.mPuzzleNewClick(Sender: TObject);
...
var
  Col, N, R, R1, C1, R2, C2, I, J, K: Integer;
  OK: Boolean;
...
begin
  ...
  // Hide all comparison static texts
  for I := 0 to 9 do
    stComparisons[I].Visible := False;
  ...
  // Randomly select numbers displayed by application (set them negative in array to recognize them)
  N := Random(iSize) + iSize + 1;                                              // count of numbers displayed in this puzzle
  for I := 1 to N do begin
    repeat
      R1 := Random(iSize); C1 := Random(iSize);
    until aGridNumbers[R1, C1] > 0;                                            // field ok, if not already selected before
    aGridNumbers[R1, C1] := -aGridNumbers[R1, C1];                             // fill negative field-value into array
  end;
  // Randomly generate comparison relationships
  N := Random(iSize - 1) + iSize; SetLength(aRelationships, N);                // count of relationships used in this puzzle
  for I := 0 to N - 1 do begin
    // Generate the relationships
    repeat
      OK := True;
      R1 := Random(iSize); C1 := Random(iSize);                                // chose random first field
      if aGridNumbers[R1, C1] < 0 then                                         // use only if field is filled in by user (arbitrarily done so)
        OK := False
      else begin
        R := Random(2);
        case R of
        ...
        if (R2 > iSize - 1) or (C2 > iSize - 1) then                           // avoid fields falling outside the grid
          OK := False
        else if aGridNumbers[R2, C2] < 0 then                                  // use only if field is filled in by user (arbitrarily done so)
          OK := False;
      end;
      if OK then begin
        if I > 0 then begin
          ...
          for J := 0 to I - 1 do begin
            if (R1 = aRelationships[J].FirstR) and (C1 = aRelationships[J].FirstC) and
           (R2 = aRelationships[J].SecondR) and (C2 = aRelationships[J].SecondC) then
           OK := False;
          end;
        end;
      end;
    until OK;
    // Set values for actual record of TRelationships array
    aRelationships[I].FirstR := R1; aRelationships[I].FirstC := C1;
    aRelationships[I].SecondR := R2; aRelationships[I].SecondC := C2;
    if Abs(aGridNumbers[R1, C1]) > Abs(aGridNumbers[R2, C2]) then
      aRelationships[I].Comparison := '>'
    else
      aRelationships[I].Comparison := '<';
  end;
  // Display the grid
  for I := 0 to iSize - 1 do begin
    for J := 0 to iSize - 1 do begin
      if aGridNumbers[I, J] > 0 then begin
        // Numbers of these fields are to be entered by user
        shGridShapes[I, J].Brush.Color := clWhite;
        stGridNumbers[I, J].Caption := '';
      end
      else begin
        // Numbers of these fields are displayed by the application
        shGridShapes[I, J].Brush.Color := clYellow;
        stGridNumbers[I, J].Caption := IntToStr(-aGridNumbers[I, J]);
      end;
    end;
  end;
  // Display comparison relationships
  for I := 0 to Length(aRelationships) - 1 do begin
    if aRelationships[I].FirstC <> aRelationships[I].SecondC then begin
      // Comparison of 2 fields in same row
      stComparisons[I].Top := shGridShapes[aRelationships[I].FirstR, aRelationships[I].FirstC].Top + 2;
      stComparisons[I].Left := shGridShapes[aRelationships[I].SecondR, aRelationships[I].SecondC].Left - 10;
      if aGridNumbers[aRelationships[I].FirstR, aRelationships[I].FirstC] > aGridNumbers[aRelationships[I].SecondR, aRelationships[I].SecondC] then
        stComparisons[I].Caption := '>'
      else
        stComparisons[I].Caption := '<';
    end
    else begin
      // Comparison of 2 fields in same column
      stComparisons[I].Left := shGridShapes[aRelationships[I].FirstR, aRelationships[I].FirstC].Left + 21;
      stComparisons[I].Top := shGridShapes[aRelationships[I].SecondR, aRelationships[I].SecondC].Top - 22;
      if aGridNumbers[aRelationships[I].FirstR, aRelationships[I].FirstC] > aGridNumbers[aRelationships[I].SecondR, aRelationships[I].SecondC] then
        stComparisons[I].Caption := '˅'
      else
        stComparisons[I].Caption := '˄';
    end;
    stComparisons[I].Visible := True;
  end;
...
procedure TfFutoshiki.mPuzzleExitClick(Sender: TObject);
...
procedure TfFutoshiki.mOptionsSize4Click(Sender: TObject);
...
procedure TfFutoshiki.mOptionsSize5Click(Sender: TObject);
...
procedure TfFutoshiki.mOptionsSize6Click(Sender: TObject);
...
{ Menu item "Help > Source and copyright": Display source and copyright of Futoshiki puzzle }

procedure TfFutoshiki.mHelpCopyrightClick(Sender: TObject);

var
  S: string;

begin
  S := 'I found Futoshiki at https://www.janko.at/. It''s not known, who invented the logic puzzle Futoshiki (Hutoshiki, Unequal). ';
  S += 'It may have been first published in the Japanese magazine "Puzzle Communication Nikoli" and is probably public domain. ';
  S += 'Concerning my "Futoshiki" PC application itself, it is freeware/open source, as all puzzle and other applications at www.streetinfo.lu.';
  MessageDlg('Futoshiki', S, mtInformation, [mbOK], 0);
end;

{ Menu item "Help > 'Rätsel und Puzzles' website": Point web browser to janko.at }

procedure TfFutoshiki.mHelpWebsiteClick(Sender: TObject);
...
procedure TfFutoshiki.mHelpAboutClick(Sender: TObject);

var
  S: string;

begin
  S := 'Logic game: Futoshiki.' + LineEnding;
  S += 'Fill the grid fields with numbers from 1 to N, in a way, that all comparison relationships are fullfilled.' + LineEnding + LineEnding;
  S += 'Version 1.1, © allu, September-November 2021.' + LineEnding;
  S += 'See also: Source and copyright.';
  MessageDlg('About "Futoshiki"', S, mtInformation, [mbOK], 0);
end;

{ Button "Done": Read user solution from form grid and check if this is a correct solution }

procedure TfFutoshiki.btDoneClick(Sender: TObject);
...
// relationships are fulfilled, and not by comparison of the user and generated grid numbers; in fact, there may be several correct solutions...
...
begin
  ...
  if Solved then begin
    for I := 0 to Length(aRelationships) - 1 do begin
      Number1 := UserNumbers[aRelationships[I].FirstR, aRelationships[I].FirstC];
      Number2 := UserNumbers[aRelationships[I].SecondR, aRelationships[I].SecondC];
      if ((aRelationships[I].Comparison = '>') and (Number1 < Number2))
        or ((aRelationships[I].Comparison = '<') and (Number1 > Number2)) then
        Solved := False;
    end;
  end;
  // Display "success or not" message
  if Solved then
    MessageDlg('Futoshiki', 'You have successfully solved this Futoshiki puzzle!', mtInformation, [mbOK], 0)
  else begin
    MessageDlg('Futoshiki', 'Sorry. Your numbers are no valid solution of this Futoshiki puzzle !', mtInformation, [mbOK], 0);
...
procedure TfFutoshiki.btShowClick(Sender: TObject);
  ...
  for I := 0 to iSize - 1 do begin
    for J := 0 to iSize - 1 do begin
      if shGridShapes[I, J].Brush.Color <> clYellow then begin
        // Do only for fields where user has to enter a number
        shGridShapes[I, J].Brush.Color := clWhite;
...
procedure TfFutoshiki.btClearAllClick(Sender: TObject);
...
procedure TfFutoshiki.bt1Click(Sender: TObject);

begin
  WriteNumber(stGridNumbers, '1', iRow, iCol);
end;

procedure TfFutoshiki.bt2Click(Sender: TObject);

begin
  WriteNumber(stGridNumbers, '2', iRow, iCol);
end;

procedure TfFutoshiki.bt3Click(Sender: TObject);

begin
  WriteNumber(stGridNumbers, '3', iRow, iCol);
end;

procedure TfFutoshiki.bt4Click(Sender: TObject);

begin
  WriteNumber(stGridNumbers, '4', iRow, iCol);
end;

procedure TfFutoshiki.bt5Click(Sender: TObject);

begin
  WriteNumber(stGridNumbers, '5', iRow, iCol);
end;

procedure TfFutoshiki.bt6Click(Sender: TObject);
...
procedure TfFutoshiki.btClearClick(Sender: TObject);
...
procedure TfFutoshiki.laGrid00Click(Sender: TObject);

begin
  SelectField(shGridShapes, 0, 0, iRow, iCol);
end;

procedure TfFutoshiki.laGrid01Click(Sender: TObject);

begin
  SelectField(shGridShapes, 0, 1, iRow, iCol);
end;

procedure TfFutoshiki.laGrid02Click(Sender: TObject);

begin
  SelectField(shGridShapes, 0, 2, iRow, iCol);
end;

procedure TfFutoshiki.laGrid03Click(Sender: TObject);

begin
  SelectField(shGridShapes, 0, 3, iRow, iCol);
end;

procedure TfFutoshiki.laGrid04Click(Sender: TObject);

begin
  SelectField(shGridShapes, 0, 4, iRow, iCol);
end;

procedure TfFutoshiki.laGrid05Click(Sender: TObject);

begin
  SelectField(shGridShapes, 0, 5, iRow, iCol);
end;

procedure TfFutoshiki.laGrid10Click(Sender: TObject);

begin
  SelectField(shGridShapes, 1, 0, iRow, iCol);
end;

procedure TfFutoshiki.laGrid11Click(Sender: TObject);

begin
  SelectField(shGridShapes, 1, 1, iRow, iCol);
end;

procedure TfFutoshiki.laGrid12Click(Sender: TObject);

begin
  SelectField(shGridShapes, 1, 2, iRow, iCol);
end;

procedure TfFutoshiki.laGrid13Click(Sender: TObject);

begin
  SelectField(shGridShapes, 1, 3, iRow, iCol);
end;

procedure TfFutoshiki.laGrid14Click(Sender: TObject);

begin
  SelectField(shGridShapes, 1, 4, iRow, iCol);
end;

procedure TfFutoshiki.laGrid15Click(Sender: TObject);

begin
  SelectField(shGridShapes, 1, 5, iRow, iCol);
end;

procedure TfFutoshiki.laGrid20Click(Sender: TObject);

begin
  SelectField(shGridShapes, 2, 0, iRow, iCol);
end;

procedure TfFutoshiki.laGrid21Click(Sender: TObject);

begin
  SelectField(shGridShapes, 2, 1, iRow, iCol);
end;

procedure TfFutoshiki.laGrid22Click(Sender: TObject);

begin
  SelectField(shGridShapes, 2, 2, iRow, iCol);
end;

procedure TfFutoshiki.laGrid23Click(Sender: TObject);

begin
  SelectField(shGridShapes, 2, 3, iRow, iCol);
end;

procedure TfFutoshiki.laGrid24Click(Sender: TObject);

begin
  SelectField(shGridShapes, 2, 4, iRow, iCol);
end;

procedure TfFutoshiki.laGrid25Click(Sender: TObject);

begin
  SelectField(shGridShapes, 2, 5, iRow, iCol);
end;

procedure TfFutoshiki.laGrid30Click(Sender: TObject);

begin
  SelectField(shGridShapes, 3, 0, iRow, iCol);
end;

procedure TfFutoshiki.laGrid31Click(Sender: TObject);

begin
  SelectField(shGridShapes, 3, 1, iRow, iCol);
end;

procedure TfFutoshiki.laGrid32Click(Sender: TObject);

begin
  SelectField(shGridShapes, 3, 2, iRow, iCol);
end;

procedure TfFutoshiki.laGrid33Click(Sender: TObject);

begin
  SelectField(shGridShapes, 3, 3, iRow, iCol);
end;

procedure TfFutoshiki.laGrid34Click(Sender: TObject);

begin
  SelectField(shGridShapes, 3, 4, iRow, iCol);
end;

procedure TfFutoshiki.laGrid35Click(Sender: TObject);

begin
  SelectField(shGridShapes, 3, 5, iRow, iCol);
end;

procedure TfFutoshiki.laGrid40Click(Sender: TObject);

begin
  SelectField(shGridShapes, 4, 0, iRow, iCol);
end;

procedure TfFutoshiki.laGrid41Click(Sender: TObject);

begin
  SelectField(shGridShapes, 4, 1, iRow, iCol);
end;

procedure TfFutoshiki.laGrid42Click(Sender: TObject);

begin
  SelectField(shGridShapes, 4, 2, iRow, iCol);
end;

procedure TfFutoshiki.laGrid43Click(Sender: TObject);

begin
  SelectField(shGridShapes, 4, 3, iRow, iCol);
end;

procedure TfFutoshiki.laGrid44Click(Sender: TObject);

begin
  SelectField(shGridShapes, 4, 4, iRow, iCol);
end;

procedure TfFutoshiki.laGrid45Click(Sender: TObject);

begin
  SelectField(shGridShapes, 4, 5, iRow, iCol);
end;

procedure TfFutoshiki.laGrid50Click(Sender: TObject);

begin
  SelectField(shGridShapes, 5, 0, iRow, iCol);
end;

procedure TfFutoshiki.laGrid51Click(Sender: TObject);

begin
  SelectField(shGridShapes, 5, 1, iRow, iCol);
end;

procedure TfFutoshiki.laGrid52Click(Sender: TObject);

begin
  SelectField(shGridShapes, 5, 2, iRow, iCol);
end;

procedure TfFutoshiki.laGrid53Click(Sender: TObject);

begin
  SelectField(shGridShapes, 5, 3, iRow, iCol);
end;

procedure TfFutoshiki.laGrid54Click(Sender: TObject);

begin
  SelectField(shGridShapes, 5, 4, iRow, iCol);
end;

procedure TfFutoshiki.laGrid55Click(Sender: TObject);
