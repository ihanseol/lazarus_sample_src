{******************************************************}
{* Maxwell bridge unit for BridgeCircuits application *}
{******************************************************}
...
procedure TfMaxwell.FormCreate(Sender: TObject);
...
procedure TfMaxwell.FormActivate(Sender: TObject);
...
{ Button "Calculate" pushed: Calculate Maxwell bridge inductance (and resistance) }

procedure TfMaxwell.btCalcClick(Sender: TObject);

var
  R1, R2, R3, R4, C1, L4: Real;
  Mess: string;
...
begin
  ...
  if Mess = '' then begin
    ...
    L4 := (C1 * R2 * R3) / rMultL;                                             // calculate L4
    R4 := (R2 * R3 / R1) / rMultR;                                             // calculate R4
...
procedure TfMaxwell.btCloseClick(Sender: TObject);
