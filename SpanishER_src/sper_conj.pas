{************************************************************}
{* Display conjugation table unit for SpanishER application *}
{************************************************************}
...
uses
  Classes, SysUtils, FileUtil, Forms, Controls, Graphics, Dialogs, StdCtrls, IniFiles, LazUTF8, conjer;
...
begin
  ...
  VConj := StringReplace(VConj, '*', '', []);
  VConj := StringReplace(VConj, '+', '', []);
  // Remove '-' at end of conjugated form
  if RightStr(VConj, 1) = '-' then
    UTF8Delete(VConj, UTF8Length(VConj), 1);
end;

{ Color display of irregularities }

function FontColor(Verb: TVerb; VerbConj: string; ColorIrreg, ColorSame, SepSuffix: Boolean): TColor;
...
begin
  ...
  if VerbConj <> 'n/a' then begin
    if ColorIrreg then begin
      ...
      else if UpperCase(Verb.Conjugation) = 'I' then begin
        // Irregular verbs
        P := UTF8Pos('$', VerbConj);
        if P > 0 then
          Colour := clRed                                                      // irregular verb suffix
        else begin
          P := UTF8Pos('#', VerbConj);
          if P > 0 then begin
            // Irregular stem suffix: use special color only if stem suffix is separated from stem
            if SepSuffix then
              Colour := clDeepPink                                             // irregular stem suffix
            else
              Colour := clDarkOrange;                                          // irregular stem
          end
          else begin
            P := UTF8Pos('*', VerbConj);
            if P > 0 then
              ...
              P := UTF8Pos('+', VerbConj);
              if P > 0 then
                Colour := clBlue;                                              // semi-irregular stem
            end;
          end;
        end;
      end;
    ...
    for T := 1 to NTenses[M] do begin
      ...
      if (Mood = 'infinitive') or (Mood = 'participle') then begin
        ...
        if Mood = 'infinitive' then begin
          fConj.edInfinitive.Text := VerbConjugated; fConj.edInfinitive.Font.Color := FontColor(Verb, VerbConj, ColorIrreg, ColorSame, SepRoot);
        end
        else begin
          if Tense = 'gerund' then begin
            fConj.edGerund.Text := VerbConjugated; fConj.edGerund.Font.Color := FontColor(Verb, VerbConj, ColorIrreg, ColorSame, SepRoot);
          end
          else begin
            fConj.edParticiple.Text := VerbConjugated; fConj.edParticiple.Font.Color := FontColor(Verb, VerbConj, ColorIrreg, ColorSame, SepRoot);
          end;
        end;
      end
      // Imperative
        ...
        for N := 1 to NNumbers do begin
          ...
          for P := 1 to NPersons do begin
            ...
            if PE <> 1 then begin
              ...
              EditImp[PE].Text := VerbConjugated; EditImp[PE].Font.Color := FontColor(Verb, VerbConj, ColorIrreg, ColorSame, SepRoot);
            end;
          end;
        end;
      end
        ...
        for N := 1 to NNumbers do begin
          ...
          for P := 1 to NPersons do begin
            ...
            if Mood = 'indicative' then begin
              EditInd[T, PE].Text := VerbConjugated; EditInd[T, PE].Font.Color := FontColor(Verb, VerbConj, ColorIrreg, ColorSame, SepRoot);
            end
            else begin
              EditSubj[T, PE].Text := VerbConjugated; EditSubj[T, PE].Font.Color := FontColor(Verb, VerbConj, ColorIrreg, ColorSame, SepRoot);
            end;
          end;
        end;
      end;
