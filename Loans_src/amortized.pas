{***************************************************************}
{* "Simple amortized interest loan" unit for Loans application *}
{***************************************************************}
...
{ Form activation ("called" from main unit): Do "Simple amortized interest loan" calculations }

procedure TfAmortized.FormActivate(Sender: TObject);
...
begin
  ...
  while M < T do begin
    ...
    Inc(M);
    InterestPaid := (rR / 12) * (rP - PrincipalTotal); InterestPaid := Round(100 * InterestPaid) / 100;
...
procedure TfAmortized.btCloseClick(Sender: TObject);
